...
class GenARTModelOutput(ModelOutput):
    ...
    len_loss: Optional[torch.FloatTensor] = None
...
class GenARTTokenizationLayer(nn.Module):
    ...
    def _generate_clustered_pattern(self, seq_len: int, density_factor: float, cluster_factor: float, device: torch.device):
        """
        Generate an ideal clustered token boundary distribution pattern.

        Args:
            seq_len (int): Sequence length.
            density_factor (float): Overall density of boundaries.
            cluster_factor (float): Clustering factor. Value in (0, 1], smaller means more clustered.
            device: a torch.device.

        Returns:
            torch.Tensor: Ideal boundary distribution pattern.
        """
        if not (0 < cluster_factor <= 1.0):
            raise ValueError("cluster_factor must be between 0 and 1.")

        # Generate standard normal random noise
        noise = torch.randn(seq_len, device=device)

        # Create a low-pass filter (Gaussian kernel) to smooth the noise for clustering effect
        # Smaller cluster_factor means larger sigma, stronger smoothing/clustering
        sigma = (1.0 / (cluster_factor + 1e-6) - 1.0) * 5.0 + 1.0
        kernel_size = int(sigma * 6)
        if kernel_size % 2 == 0:
            kernel_size += 1

        # Use 1D Gaussian kernel
        kernel = torch.exp(
            -torch.arange(-(kernel_size // 2), kernel_size // 2 + 1, device=device).float() ** 2 / (2 * sigma ** 2))
        kernel = kernel / kernel.sum()
        kernel = kernel.view(1, 1, -1)  # [out_channels, in_channels, kernel_size]

        # Smooth the noise by 1D convolution
        smoothed_noise = F.conv1d(noise.view(1, 1, -1), kernel, padding='same').view(-1)

        # Binarize the smoothed noise according to density
        num_boundaries = int(seq_len * density_factor)
        if num_boundaries == 0:
            return torch.zeros(seq_len, device=device)

        # Find the threshold to select top-k values
        kth_value, _ = torch.kthvalue(smoothed_noise, seq_len - num_boundaries)
        threshold = kth_value

        pattern = (smoothed_noise >= threshold).float()

        return pattern

    def compute_length_loss(self, M, special_tokens_mask=None, density_factor=None, cluster_factor=None):
        """
        M: boundary tensor, shape (B, seq_len-1), value 1 means boundary at this position.
        ...
        cluster_factor: task-specific target clustering factor.
        """
        if density_factor is None:
            density_factor = 0.5  # Default density for pretraining

        # If cluster factor is not provided, fall back to uniform distribution
        if cluster_factor is None:
            cluster_factor = 1.0

        batch_size, seq_len_m = M.shape
        loss = 0.0

        # Generate ideal clustered distribution template
        ideal_pattern = self._generate_clustered_pattern(
            seq_len_m,
        ...
        for i in range(batch_size):
            ...
            if special_tokens_mask is not None:
                valid_mask = (special_tokens_mask[i] == 0)
                row = row[valid_mask]
                ideal = ideal_pattern[valid_mask]
            else:
                ideal = ideal_pattern

            row_sum = torch.sum(row)
            ideal_sum = torch.sum(ideal)

            if row_sum > 0 and ideal_sum > 0:
                row_dist = row / row_sum
                ideal_dist = ideal / ideal_sum
                loss += torch.nn.functional.mse_loss(row_dist, ideal_dist)

        return loss / batch_size if batch_size > 0 else torch.tensor(0.0, device=M.device)
    ...
    def forward(self, hidden_states: torch.Tensor, 
        ...
        len_loss = self.compute_length_loss(1-merge_decisions, special_tokens_mask=special_tokens_mask[:, 1:], 
                                            density_factor=density_factor, cluster_factor=cluster_factor)
        ...
        return new_hidden_states, new_attention_mask, new_special_tokens_mask, len_loss
...
class GenARTModel(GenARTPreTrainedModel):
    ...
    def forward(
        ...
        hidden_states, token_attention_mask, token_special_tokens_mask, len_loss = self.conversion_layer(
                hidden_states,
                attention_mask=nucleotide_attention_mask,
